...
class AscendHybridKVCacheCoordinator(HybridKVCacheCoordinator):
    ...
    def __init__(
        ...
        if enable_caching:
            # Match upstream: use raw block_size from spec for the divisibility
            # check. _get_effective_block_size may include compress_ratio or
            # DCP/PCP scaling that makes the effective size non-divisible,
            # but the BlockHashListWithBlockSize wrapper handles that at
            # lookup time, so the raw block_size is the correct check target.
            assert all(

...
class Eagle3LlamaForCausalLM(LlamaForCausalLM):
    ...
    def compute_logits(
        ...
        # import os
        # # ==================== store vocab mapping  ====================
        # save_dir = "/opt/w00958190/eagle3_gabbages/vocab_mapping/"

        # save_path = os.path.join(save_dir, "qwen3_32b_eagle3_vocabmapping.pt")

        # if not os.path.exists(save_path):
        #     os.makedirs(save_dir, exist_ok=True)

        #     d2t_tensor = self.draft_id_to_target_id.detach().cpu().to(torch.int64)

        #     t2d_tensor = torch.zeros(
        #         self.config.vocab_size, 
        #         dtype=torch.bool, 
        #         device=targets.device
        #     )
        #     t2d_tensor[targets] = True 
        #     t2d_tensor = t2d_tensor.detach().cpu()
            
        #     mapping_dict = {
        #         "d2t": d2t_tensor,
        #         "t2d": t2d_tensor
        #     }
        #     torch.save(mapping_dict, save_path)
        #     print(f"📦 [Eagle3 Debug] 成功提取并保存 Vocab Mapping 至: {save_path}")
        #     print(f"   - d2t shape: {mapping_dict['d2t'].shape}, dtype: {mapping_dict['d2t'].dtype}")
        #     print(f"   - t2d shape: {mapping_dict['t2d'].shape}, dtype: {mapping_dict['t2d'].dtype}")
        # # =======================================================================
